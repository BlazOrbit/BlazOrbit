...
public sealed class WasmTestContext : BlazorTestContextBase
{
    ...
    protected override void ConfigureScenarioServices(IServiceCollection services) =>
        services.AddBlazOrbitLocalizationWasm(); //services.AddSingleton<ITestHostingModel>(new WasmHostingModel());// Localization WASM//services.AddSingleton<ITestCultureService, FakeWasmCultureService>();
}