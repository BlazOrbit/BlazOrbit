...
public class BOBHotkeyServiceTests
{
    ...
    public async Task Should_Invoke_Handler_For_Matching_Combo()
    {
        ...
        int hits = 0;
        using IDisposable _ = svc.Register("ctrl+s", "Save", () =>
        {
            hits++;
    ...
    public async Task Should_Stop_Firing_After_Dispose()
    {
        IHotkeyService svc = new HotkeyService();
        int hits = 0;
        IDisposable reg = svc.Register("escape", "Close", () =>
        {
            hits++;
            return Task.CompletedTask;
        });
    ...
    public async Task Should_Normalize_Modifier_Order()
    {
        ...
        IHotkeyService svc = new HotkeyService();
        int hits = 0;
        using IDisposable _ = svc.Register("Shift + Ctrl + S", "Save", () =>
        {
            hits++;
            return Task.CompletedTask;
        });
    ...
    public async Task Should_Fire_Multiple_Handlers_For_Same_Combo()
    {
        IHotkeyService svc = new HotkeyService();
        int a = 0, b = 0;
        using IDisposable _1 = svc.Register("ctrl+s", "A", () =>
        {
            a++;
            return Task.CompletedTask;
        });
        using IDisposable _2 = svc.Register("ctrl+s", "B", () =>
        {
            b++;
            return Task.CompletedTask;
        });
    ...
    public void Should_List_Registered_Hotkeys_For_Cheat_Sheet()
    {
        ...
        svc.RegisteredHotkeys.Should()
            .Contain(d => d.Combo == "ctrl+s" && d.Description == "Save" && d.Scope == HotkeyScope.Global);
        svc.RegisteredHotkeys.Should().Contain(d =>
            d.Combo == "ctrl+k" && d.Description == "Command palette" && d.Scope == HotkeyScope.Page);
    }
