...
* **Form inputs that extends native capabilities with 17 input components**
  > Text, Textarea, Number, Password, Checkbox, Switch, Radio, Dropdown (searchable + multi-select + tree), File upload, Color picker, Date/time, Date range, Autocomplete, OTP, Number-slider, Range-slider.
All them **Support for `EditForm`, `DataAnnotationValidator` and `FluentValidation`**  

* **Display**
  >  Avatar, Banner, Chip, Progress, Rating, Stat Card, Badge, Carousel and Icons ([+10.000 icons](https://localhost:7223/features/icons))
...
* **Charts** (by extension with `BlazOrbit.Charts` package)
  SVG-rendered. No dependencies.
...
* **Optional Localization Integration** (by extension with `BlazOrbit.Localization.*` package)
  > BlazOrbit components are localized. It provides a fully optimized localization system, based on source generators, that integrates with native IStringLocalizer.
  > [Learn more](https://blazorbit.com/features/localization)
  
*  **Also provided `.skill` bundle that teaches AI coding
agents how to work with BlazOrbit.**

* **Customizable Theme and Design token system**
  > [Theme Generator](https://blazorbit.com/utils/themegenerator)
...
In cases such as form components, it uses InputBase just like native components. But it improves the usability of native components and adds features (Styling, FluentValidation...).*
