//#region Types/Pattern/PatternInterop.ts
var patternInstances = /* @__PURE__ */ new Map();
function initialize(container, dotnetRef, componentId) {
...
async function handleInput(e) {
	...
	try {
		...
		if (value.length === maxLength) {
			if (await instance.dotnetRef.invokeMethodAsync("OnSpanComplete", index, value)) moveToNextSpan(instance, index);
		}
	} catch (error) {
...
async function handleKeyDown(e) {
	...
	const index = getSpanIndex(target);
	if (e.key === "Tab") {
		const spans = Array.from(instance.container.querySelectorAll("[contenteditable=\"true\"], [data-bob-toggle=\"true\"]"));
		const currentIdx = spans.findIndex((s) => s === target);
		const isFirst = currentIdx === 0;
		if (currentIdx === spans.length - 1 && !e.shiftKey || isFirst && e.shiftKey) return;
		e.preventDefault();
		let nextSpan = null;
		if (e.shiftKey && currentIdx > 0) nextSpan = spans[currentIdx - 1];
		else if (!e.shiftKey && currentIdx < spans.length - 1) nextSpan = spans[currentIdx + 1];
		if (nextSpan) {
			nextSpan.focus();
			if (isEditableSpan(nextSpan)) selectSpanContentInternal(nextSpan);
		}
...
function moveToNextSpan(instance, currentIndex) {
	const nextSpan = Array.from(instance.container.querySelectorAll("[contenteditable=\"true\"]")).find((span) => {
		return getSpanIndex(span) > currentIndex;
	});
	if (nextSpan) setTimeout(() => {
		nextSpan.focus();
		selectSpanContentInternal(nextSpan);
	}, 0);
}
function moveToPrevEditableSpan(instance, currentIndex) {
	const prevSpan = [...Array.from(instance.container.querySelectorAll("[contenteditable=\"true\"]"))].reverse().find((span) => {
		return getSpanIndex(span) < currentIndex;
	});
	if (prevSpan) {
		prevSpan.focus();
...
function updateSpanValue(componentId, index, value) {
	const instance = patternInstances.get(componentId);
	if (!instance) return;
	const span = instance.container.querySelector(`[data-bob-index="${index}"]`);
	if (span && span.textContent !== value) {
		const isFocused = document.activeElement === span;
		span.textContent = value;
		if (isFocused && isEditableSpan(span)) selectSpanContentInternal(span);
	}
}
function selectSpanContent(componentId, index) {
	const instance = patternInstances.get(componentId);
	if (!instance) return;
	const span = instance.container.querySelector(`[data-bob-index="${index}"]`);
	if (span) selectSpanContentInternal(span);
}
function setCaretToEnd(componentId, index) {
	const instance = patternInstances.get(componentId);
	if (!instance) return;
	const span = instance.container.querySelector(`[data-bob-index="${index}"]`);
...
function focusSpan(componentId, index) {
	const instance = patternInstances.get(componentId);
	if (!instance) return;
	const span = instance.container.querySelector(`[data-bob-index="${index}"][contenteditable="true"]`);
	if (span) {
		span.focus();
		selectSpanContentInternal(span);
	}
}
function focusFirstEditable(componentId) {
	const instance = patternInstances.get(componentId);
	if (!instance) return;
	const firstSpan = instance.container.querySelector("[contenteditable=\"true\"]");
	if (firstSpan) {
		firstSpan.focus();
...
function getInstanceFromElement(element) {
	const container = element.closest("[data-bob-pattern-id]");
...
//#endregion
export { dispose, focusFirstEditable, focusSpan, initialize, selectSpanContent, setCaretToEnd, updateSpanValue };

//# sourceMappingURL=PatternInterop.min.js.map