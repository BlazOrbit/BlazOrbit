...
public class BOBHeatmapChart<TX, TY> : BOBChartBase<TX, TY>
    where TX : notnull
    where TY : notnull
{
    /// <summary>Cells; each cell defines its column, row and intensity value.</summary>
    [Parameter] public IEnumerable<BOBChartHeatmapCell<TX, TY>>? Cells { get; set; }

    /// <summary>Optional explicit column order; otherwise auto-derived from cells.</summary>
    [Parameter] public IEnumerable<TX>? Columns { get; set; }

    /// <summary>Optional explicit row order; otherwise auto-derived from cells.</summary>
    [Parameter] public IEnumerable<TY>? Rows { get; set; }

    /// <summary>Lower-bound color of the ramp (mapped to min cell value).</summary>
    [Parameter] public string LowColor { get; set; } = "#e0f2fe";

    /// <summary>Upper-bound color of the ramp (mapped to max cell value).</summary>
    ...
    protected override void RenderSvg(RenderTreeBuilder builder)
    {
        ...
        if (Math.Abs(maxV - minV) < double.Epsilon) maxV = minV + 1;
        ...
        {
            if (!colIdx.TryGetValue(cell.X, out int c) || !rowIdx.TryGetValue(cell.Y, out int r)) continue;
